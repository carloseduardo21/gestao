...
export function UpdateProduct({ product }: IProps) {
  const { push } = useRouter()
  const id = product.id
  const [description, setDescription] = useState(product.description)
  const [brand, setBrand] = useState(product.brand)
  const [costPrice, setCostPrice] = useState(
    parseFloat(product.costPrice).toFixed(2).replace('.', ','),
  )
  const [salePrice, setSalePrice] = useState(
    parseFloat(product.salePrice).toFixed(2).replace('.', ','),
  )
  const [codeBar, setCodeBar] = useState(
    product.codeBar === null ? '' : product.codeBar,
  )
  const [refer, setRefer] = useState(
    product.refer === null ? '' : product.refer,
  )
  const [unit, setUnit] = useState(product.unit)
  const [min, setMin] = useState(
    product.Stock.length > 0 ? product.Stock[0].min : 0,
  )
  const [max, setMax] = useState(
    product.Stock.length > 0 ? product.Stock[0].max : 0,
  )
  ...
  const formSubmit = async (e: FormEvent) => {
    ...
    await APIWeb.put(`/products/${id}`, data)
      .then((response) => {
